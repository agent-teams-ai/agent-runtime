...
treats as an inward leak from `application/`. Linux route binding and product
route qualification now live under matching `src/features/` directories with
curated `index.ts` / `internal.ts` entrypoints. Host composition re-exports
those feature entries from the matching `src/composition/` files, so existing
composition and test imports keep working. The implementations stay at the
feature root rather than under `application/`: they still import Agent
Execution / Provider Access composition types, which L0 treats as an inward
leak from `application/`. Trusted runtime-access scope copying now lives under
...
leak from `application/`. Process lifecycle,
