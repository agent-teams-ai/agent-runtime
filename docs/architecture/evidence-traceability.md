...
| `spikes/docker-linux-enforced-network-route-results.md` | Docker/Linux Codex post-claim ordering, exclusive route admission, first-write adjacency, capability minting, and the two-fact product gate | An enforced network route is admitted only by its own installed lease: route admission is the last gate before provider execution, first-write consumption is adjacent to the emitted bytes, and a contained turn composes only when an authentic route-enforcement capability and an exact registry target agree. |
