...
const callExpressionRecord = (node, source) => {
  ...
  if (!directRequire && !requireResolve) return undefined;
...
const profileShapeIssues = (profile, profilePath) => {
  ...
  if (!validateProfileShape(profile)) {
    for (const error of validateProfileShape.errors ?? []) {
      if (error.instancePath.startsWith("/authority")) continue;
...
const sameValues = (actual, expected) => {
  ...
  return actual.length === expected.length && [...actual].sort(compareText).every((value, index) => value === sortedExpected[index]);
};
...
const validateProfile = (profile, profilePath) => {
  ...
  for (const field of ["productionRoots", "outOfScope"]) if (!Array.isArray(profile.scope?.[field])) issues.push(issue("FM_PROFILE_INVALID", profilePath, 1, `scope.${field} must be an array`));
  for (const field of ["moduleRoles", "features", "assemblyFiles", "featureEdges", "extensions", "deviations", "exceptions"]) if (!Array.isArray(profile[field])) issues.push(issue("FM_PROFILE_INVALID", profilePath, 1, `${field} must be an array`));
...
const detectCycles = (edges, edgeLocations, kind) => {
  ...
  for (const node of [...adjacency.keys()].sort()) visit(node, [], new Set());
...
const featureStructureIssues = async (feature, allPaths, absoluteRoot) => {
  ...
  for (const role of feature.roles ?? []) {
    if (!featureFiles.some((path) => layerForPath(feature, path) === role)) issues.push(issue("FM_EMPTY_LAYER", `${feature.root}/${role}`, 1, `${feature.id} declares empty ${role} layer`));
  }
  return issues;
};
...
const parseFileImports = async (absoluteFile, path) => {
  ...
  if (!parsed.errors?.length) return { imports: importRecords(parsed.program, source), issues: [] };
  return {
    imports: [],
...
const inspectImport = (context) => {
  const { absoluteFile, absoluteRoot, features, isAssembly, sourceFeature, path, imported } = context;
  if (imported.nonliteral) return [issue("FM_NONLITERAL_LOADING", path, imported.line, `${imported.syntax} requires a string literal`)];
  const target = resolveLocalImport(slash(absoluteFile), imported.specifier);
  if (!target) return [];
  ...
  const entrypointIssues = publicEntrypointIssues(importContext);
...
export const checkFeatureModules = async ({ root = REPOSITORY_ROOT, profilePath = DEFAULT_PROFILE, requiredStatus } = {}) => {
  ...
  const issues = validateProfile(profile, profilePath);
  if (issues.some((candidate) => candidate.code === "FM_PROFILE_INVALID")) return issues.sort(compareIssues);
  ...
  for (const absoluteFile of allFiles) {
    ...
    for (const imported of parsed.imports) issues.push(...inspectImport({ absoluteFile, absoluteRoot, features, isAssembly, sourceFeature, path, imported, declaredEdges, observedEdges, edgeLocations }));
  }
