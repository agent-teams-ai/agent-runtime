...
export function inspectArchive(tgz) {
  ...
  while (offset + 512 <= tar.length) {
    ...
    if (!Number.isSafeInteger(size) || size > MAX_FILE || files.size >= MAX_MEMBERS) throw new Error("archive: member size or count limit");
    const next = offset + 512 + Math.ceil(size / 512) * 512;
    if (next > tar.length) throw new Error("archive: truncated member");
