...
export function installedFiles(base, key, onEnumerated = () => {}) {
  ...
  let entries = 0;
  let total = 0;
  function visit(descriptor, prefix, depth) {
    ...
    if (entries + items.length > MAX_INSTALLED_ENTRIES) throw new Error("installed: member count limit");
    ...
    directories.push({ descriptor, state: before, names, path: prefix });
    onEnumerated(prefix);
    for (const [index, item] of items.entries()) {
      if (++entries > MAX_INSTALLED_ENTRIES) throw new Error("installed: member count limit");
      ...
      if (item.isDirectory()) {
        ...
        directories.push({ descriptor: child, parent: descriptor, name: item.name, snapshot: snapshots[index], path });
        ...
        if (stat.size > MAX_INSTALLED_FILE || total + stat.size > MAX_INSTALLED_TOTAL) throw new Error("installed: byte limit");
        total += stat.size;
        pending.push({ file, parent: descriptor, name: item.name, stat, path });
      }
  ...
  try {
    ...
    try {
      visit(root, "", 0);
      for (const { file, parent, name, stat, path } of pending) {
      ...
      for (const { descriptor, parent, name, snapshot, state, names, path } of directories) {
        ...
        if (state && (!sameDirectoryState(state, directoryState(descriptor)) ||
            names.join("\0") !== boundedDirectoryItems(descriptor).map(item => item.name).sort().join("\0") ||
            !sameDirectoryState(state, directoryState(descriptor)))) {
