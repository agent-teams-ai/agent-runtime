...
test('PR71 reviewed owners and helpers retain exact live relationships without expanding adoption', async () => {
  ...
  const helper = pending.boundaries.find(b => b.id === 'composition.embedded-runtime.contained-turn-routing').relationships
    .filter(edge => edge.from.endsWith('/features/linux-codex-node-recipe/adapters/linux-codex-node-recipe-consumption.ts'));
  ...
  const darwinAuthority = pending.boundaries.find(b => b.id === 'composition.embedded-runtime.contained-turn-routing').relationships
    .filter(edge => edge.from.endsWith('/features/darwin-contained-turn-authority/adapters/darwin-contained-turn-authority.ts'));
