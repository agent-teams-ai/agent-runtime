...
const outsideFeatureImportIssues = ({ imported, isAssembly, path, sourceFeature, targetPath, productionRoots, assemblyFiles }) => {
  ...
  const sourceLayer = sourceFeature && !Object.values(sourceFeature.entrypoints ?? {}).includes(path)
    ? layerForPath(sourceFeature, path)
    : undefined;
  if (isHostCompositionAssembly(targetPath, assemblyFiles?.has(targetPath)) && ["adapters", "composition"].includes(sourceLayer)) {return [];}
...
const acceptLegacyCompositionSeam = (context, targetPath) => {
  const {sourceFeature, path, imported, isAssembly} = context;
  const seam = context.compositionDependencies?.find(edge => edge.from === path && edge.to === targetPath && edge.kind === imported.kind);
  if (!seam || !(sourceFeature && layerForPath(sourceFeature, path) === "composition" || isAssembly && seam.facade === true)) {return false;}
  context.observedCompositionDependencies?.add(JSON.stringify(seam));
  return true;
};
...
const inspectImport = (context) => {
  ...
  if (acceptLegacyCompositionSeam(context, targetPath)) {return [];}
...
export const checkFeatureModules = async ({ root = REPOSITORY_ROOT, profilePath = DEFAULT_PROFILE, requiredStatus, acceptedDecisions } = {}) => {
  ...
  findings.add(await packagePolicyIssues({
    ...
    localPackageImports,
    featureForPath,
    assemblyFiles: profile.assemblyFiles,
  }));
...
// Reused by additive, accepted source slices; legacy module activation stays unchanged.
export const scopedFeaturePrimitives = { issue, collectProductionFiles, featureForPath, layerForPath,
  ownershipIssues, parseFileImports, assemblyGrammarIssues, inspectImport, featureStructureIssues,
  compareIssues };
