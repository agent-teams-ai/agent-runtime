...
const sameIdentity = (a, b) => ['dev', 'ino', 'mode', 'uid', 'gid'].every(key => a[key] === b[key]);
...
function reader(root) {
  ...
  function directory(path, parent, identityOnly = false) {
    const fd = openSync(path, constants.O_RDONLY | constants.O_DIRECTORY | constants.O_NOFOLLOW);
    handles.push(fd);
    const stat = fstatSync(fd, {bigint: true});
    requireEvidence(stat.isDirectory() && (!parent || stat.dev === parent.dev));
    bindings.push({path, fd, stat, identityOnly});
    return fd;
  }
  ...
  try {
    ...
    let fd = directory('/', undefined, true);
    for (const part of root.slice(1).split('/')) {fd = directory(child(fd, part), undefined, true);}
    directories.set('', fd);
  } catch (error) {for (const fd of handles.toReversed()) {closeSync(fd);} throw error;}
  return {
    ...
    verify() {
      for (const {path, fd, stat, identityOnly} of bindings) {
        requireEvidence((identityOnly ? sameIdentity : same)(stat, lstatSync(path, {bigint: true})));
        if (fd !== undefined) {requireEvidence((identityOnly ? sameIdentity : same)(stat, fstatSync(fd, {bigint: true})));}
      }
    },
    close() {for (const fd of handles.toReversed()) {closeSync(fd);}},
  };
}

function matchMarker(manifest, blobs, {approval, turn, resultRef, manifestDigest}) {
...
function verifyManifest(fs, manifestDigest, {operationId, approval, turn, records}) {
  ...
  for (const item of [...manifest.entries.filter(entry => entry.kind === 'file'), ...manifest.output]) {
  ...
  const {fileMatches, finalMatches} = matchMarker(manifest, blobs, {approval, turn, resultRef, manifestDigest});
...
function validateCollection(approval, operationId, observations) {
...
function latestTurn(observations, operationId) {
  let turn;
...
export function collectLinuxCodexLiveEvidence({root, approval, operationId, observations}) {
  ...
  let fs;
  try {
    validateCollection(approval, operationId, observations);
    const turn = latestTurn(observations, operationId);
    fs = reader(root);
