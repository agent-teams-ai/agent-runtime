...
import {operationNetworkLabels} from
...
function fixture() {
  ...
  const raw = {Id: id, Name: recipe.name, Driver: "bridge", Scope: "local", Internal: true,
    Attachable: false, Ingress: false, EnableIPv6: false, Containers: {},
    Labels: operationNetworkLabels(recipe.binding, "a".repeat(64)),
  ...
  const command: FirewallCommand = async (tool, argv) => {
    ...
    if (tool === "ip") {return JSON.stringify([{ifname: "br-bbbbbbbbbbbb", linkinfo: {info_kind: "bridge"},
  ...
  const listener = decorate(base, subject);
  const cut = new AbortController();
  return {state, raw, listener, cut, open: () => listener.open(async () => {}, cut), recipe, address};
}

test("inert construction, bound endpoint, name then ID inspection, listener-first exact cleanup", async () => {
  const f = fixture(); assert.deepEqual(f.state.events, []);
  const opened = await f.open();
  assert.deepEqual(opened.address, f.address);
  assert.ok(f.state.events[1]!.endsWith(f.recipe.name));
  assert.ok(f.state.events.some(e => e.endsWith("network inspect " + "b".repeat(64))));
...
test("failed bind makes no command; failed install retains rollback handle for retry", async () => {
  const bind = fixture(); bind.state.failOpen = true;
  ...
  const f = fixture(); f.state.failInsert = true; f.state.deleteFails = true;
  await assert.rejects(f.open());
...
test("abort after insertion refuses publication and removes only retained rule", async () => {
  const f = fixture(); f.state.afterInsert = () => f.cut.abort();
  await assert.rejects(f.open());
  assert.equal((await f.listener.close()).state, "closed"); assert.equal(f.state.rule.length, 0);
});

test("close during pending lookup joins opening; uncertain listener close remains retryable", async () => {
  const f = fixture(); const entered = Promise.withResolvers<void>(); const resume = Promise.withResolvers<void>();
  ...
  const uncertain = fixture(); await uncertain.open(); uncertain.state.closeState = "unknown";
...
test("lookup substitution and operation-label mismatch cannot install a rule", async () => {
  for (const mutate of [(f: ReturnType<typeof fixture>) => {f.raw.Name = "other";},
    (f: ReturnType<typeof fixture>) => {f.raw.Labels = {...f.raw.Labels, "com.agent-runtime.http.operationSha256": "c".repeat(64)};}]) {
    const f = fixture(); mutate(f); await assert.rejects(f.open());
