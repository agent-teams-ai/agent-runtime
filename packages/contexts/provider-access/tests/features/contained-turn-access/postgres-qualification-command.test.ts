...
const probe = (databaseUrl?: string) => spawnSync(process.execPath,
  ["--import", preload, "--input-type=module", "-e", "process.stdout.write('test-runner-started')"], {
    env: databaseUrl === undefined ? {} : {PA_POSTGRES_DISPOSABLE_URL: databaseUrl},
    encoding: "utf8", timeout: 5_000,
  });

test("the mandatory PA PostgreSQL gate fails before the runner when its database is missing", () => {
  for (const value of [undefined, "", "   "]) {
    const result = probe(value);
    assert.equal(result.status, 1);
    assert.equal(result.stdout, "");
    assert.match(result.stderr, /qualification requires PA_POSTGRES_DISPOSABLE_URL/u);
  }
});

test("an explicit database permits the runner; the actual suite owns disposable-scope validation", () => {
  const result = probe("postgresql://fixture:fixture@127.0.0.1:5433/ar69_pa_test_ci");
