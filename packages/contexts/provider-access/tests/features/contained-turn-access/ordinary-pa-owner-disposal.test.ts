...
test(`pending capture failure does not starve real PA owner ${boundary} retries or independent capture disposal`, async () => {
  ...
  // Direct disposal starts before the settled observer microtask: pending capture
  // two needs both cancellation and the post-settlement cleanup confirmation.
  ...
  await Promise.all(pending); assert.deepEqual(pendingCalls, [2, 2]);
