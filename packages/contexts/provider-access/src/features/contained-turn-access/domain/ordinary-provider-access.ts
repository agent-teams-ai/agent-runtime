...
export function snapshotOrdinaryPaBinding(input: unknown): OrdinaryPaBinding {
  if (input === null || typeof input !== 'object' || Object.getPrototypeOf(input) !== Object.prototype) { throw new OrdinaryPaUnavailable(); }
  const fields = Object.getOwnPropertyDescriptors(input);
  if (Reflect.ownKeys(fields).length !== keys.length || keys.some(key => !fields[key] || !('value' in fields[key]!))) { throw new OrdinaryPaUnavailable(); }
  const get = (key: typeof keys[number]) => {
    const value: unknown = fields[key]?.value;
    if (typeof value !== 'string' || value.length < 1 || value.length > 512 || Array.from(value).some(character => character.charCodeAt(0) < 32 || character.charCodeAt(0) === 127)) { throw new OrdinaryPaUnavailable(); }
    return value;
  };
  if (get('executionProfile') !== 'user-session-v1' || get('effectClass') !== 'ordinary_user_session_effect' ||
      get('capabilityManifestRevision') !== 'ordinary-codex-macos-arm64-0.153.4-v1') { throw new OrdinaryPaUnavailable(); }
