...
test("binds exact route, credential, committed authority receipt, canonical request, policy, and peer", async () => {
  ...
  assert.deepEqual(fixture.events, ["route:resolve", "policy:resolve", "dispatch:observe", "route:revalidate", "transport:open", "transport:execute",
  ...
  assert.deepEqual(body.dispatchReceipt, receipt()); assert.deepEqual(fixture.dispatchInputs, [dispatch, dispatch]);
...
test("redacts query targets and binds versioned path, request, and exact wire digests", async () => {
  ...
  const providerBindingDigest = containedTurnEgressProviderBindingDigest(route({pathConstraint: path}));
  const fixture = harness({route: route({pathConstraint: path}), dispatchOutcome: Object.freeze({status: "consumed",
  ...
  const outcome = await createContainedTurnEgressGateway(host(), fixture.dependencies)
    .exchange(request({path, dispatch: {...dispatch, providerBindingDigest}}));
...
test("disposal cancels owner waits and quarantines unsettled acquisition or close; late transport closes once", async () => {
  for (const stage of ["open", "callback", "execute", "close"] as const) {
    ...
    const pending = gateway.exchange(request()); const marker = stage === "open" ? "transport:open" : stage === "callback" ?
      "transport:ready" : stage === "execute" ? "policy:revalidate" : "transport:close";
...
test("all async owner callbacks can await reentrant disposal across settlement without a self-flight deadlock", {timeout: 5_000}, async () => {
  for (const stage of ["route-resolve", "policy-resolve", "dispatch", "policy-revalidate", "route-revalidate", "dispatch-final", "route-final", "open", "execute", "close"]) {
    ...
    const targets: Record<string, [object, string]> = {
      ...
      "dispatch-final": [fixture.dependencies.dispatchAuthority, "observeDispatchConsumption"],
      "route-final": [fixture.dependencies.routeAuthority, "revalidateExact"],
    ...
    let calls = 0; owner[method] = async (...args: unknown[]) => {
      calls += 1; if (stage.endsWith("-final") && calls === 1) {return await Reflect.apply(original, target, args);}
    ...
    assert.equal(fixture.closeCount, ["route-resolve", "policy-resolve", "dispatch", "route-revalidate"].includes(stage) ? 0 : 1, stage);
  }
});
...
test("dispatch consumption at claimBefore is expired while the immediately preceding time is valid", async () => {
  for (const consumedAtControlTime of [199, 200, 201]) {
    ...
    assert.equal(fixture.closeCount, consumedAtControlTime === 199 ? 1 : 0);
  }
});
...
test("a 253-character DNS host remains valid and the full Host header counts against the wire budget", async () => {
  ...
  for (const delta of [0, -1]) {
    const providerBindingDigest = containedTurnEgressProviderBindingDigest(route({host: hostname, tlsServerName: hostname}));
    const fixture = harness({route: route({host: hostname, tlsServerName: hostname}), observation: observation({tlsServerName: hostname}),
      dispatchOutcome: Object.freeze({status: "consumed", lifecycleState: "claim_committed", receipt: receipt({providerBindingDigest})})});
    const bytes = wire(request(), hostname).byteLength;
    const outcome = await createContainedTurnEgressGateway(host(), fixture.dependencies)
      .exchange(request({dispatch: {...dispatch, providerBindingDigest}, budgets: {requestBytes: bytes + delta, responseBytes: 2_000, deadlineMs: 200}}));
