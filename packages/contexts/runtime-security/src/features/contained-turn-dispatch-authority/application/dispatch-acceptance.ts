...
export const acceptanceCanonical = (value: unknown): string => {
  if (value === null || typeof value !== 'object') {
    ...
    if (typeof encoded !== "string") {throw new TypeError('invalid acceptance value');}
    return encoded;
  }
...
export const policyPermits = (policy: DispatchAcceptancePolicy, intent: DispatchAcceptanceIntent,
  now: number): boolean => {
  const candidate: {readonly enabled: unknown; readonly revoked: unknown} = policy;
  return snapshotExactDispatchRecord(policy, ['scope', 'providerId', 'intentDigest', 'policyRevision',
  ...
  candidate.enabled === true && candidate.revoked === false &&
