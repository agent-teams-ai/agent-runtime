...
export const postClaimFixture = async (t: TestContext, gateway?: string) => {
  ...
  const engine: DockerEnginePort = {
    ...
    async remove() {record("remove"); state.removed = true;},
  ...
  const listener = {
    ...
    sealAdmission() {physical.seals += 1;},
  ...
  const dependencies: Dependencies = {
    ...
    deadlines: {engineIdentityMs: 5_000, allocationMs: 5_000, launchMs: 5_000, membershipMs: 5_000, cleanupMs: 5_000},
    routeAdmission: {async admit(input) {
      record("route-admission"); routeAdmissions.push(input);
      return {kind: "unsupported", reason: "owner"};
    }},
  };
  ...
  return {network, subject, proof, claimed, controller, dependencies, events, faults, hooks, state, v4Storage,
    physical, policies, routeAdmissions, lifecycle, engineCall, get journal() {return journal;}};
};
