...
export const captureNativeQualification = (value, platform, architecture) => {
  ...
  choice(input.recipe, [platform === "linux" ? "linux-x64-clang-shared/v1" : "darwin-arm64-clang-bundle/v1"]);
  ...
  const inputs = record(input.inputs, ROLES);
...
// The separately pinned helper implements these two recipes; its exact bytes
...
export const nativeInvocation = native => Object.freeze({
  args: Object.freeze(["scripts/build-native-helper.mjs", "--qualified", native.recipe,
    ...ROLES.map(role => native.inputs[role].path), native.deploymentTarget]),
...
export const matchNativeQualification = async (snapshot, qualification) => {
  ...
  for (const role of ROLES) {
    ...
    const file = role === "compiler" || role === "linker";
