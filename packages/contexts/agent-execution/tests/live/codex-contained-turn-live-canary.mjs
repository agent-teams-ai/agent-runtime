...
const readBoundCredentialOutputInventory = async (codexHome, input, expectedInventory) => {
  const { bindCodexCanaryOutputInventory, readCodexCanaryCredentialInventory } = await import("./codex-canary-credential-inventory.mjs");
...
const run = async (executionProvenance, canaryProviderAuthorities, runObservation, state) => {
  ...
  const { readCodexCanaryCredentialInventory } = await import("./codex-canary-credential-inventory.mjs");
  ...
  state.candidateEvidence = prepareCandidateEvidence(platformTuple, executionProvenance);
