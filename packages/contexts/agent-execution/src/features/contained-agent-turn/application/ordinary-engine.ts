...
export const createOrdinaryEngine = (dependencies: OrdinaryTurnDependencies): OrdinaryFeature => {
  ...
  let disposed = false;
  const execute = async (initial: OrdinaryOperation, controller: AbortController): Promise<OrdinaryOperation> => {
  ...
  const submitAccepted = async (input: OrdinaryInput, options?: OrdinarySubmitOptions): Promise<OrdinarySubmitOutcome> => {
    ...
    flights.set(accepted.operation.operationId, {operation: accepted.operation, controller, completion});
    const signalCancellations: Promise<void>[] = [];
    const cancel = (): void => {
      const pending = dependencies.operationStore.cancel(reference(accepted.operation)).then(() => {controller.abort();}, () => {controller.abort(); throw new Error("ordinary signal cancellation persistence unresolved");});
      signalCancellations.push(pending); void pending.catch(() => {});
    };
    options?.signal?.addEventListener("abort", cancel, {once: true});
    try {
      if (disposed || options?.signal?.aborted) {await dependencies.operationStore.cancel(reference(accepted.operation)); controller.abort();}
      try {options?.onAccepted?.(reference(accepted.operation));} catch {await dependencies.operationStore.cancel(reference(accepted.operation)); controller.abort();}
      return {status: "observed", turn: view(await execute(accepted.operation, controller))};
    } finally {
      options?.signal?.removeEventListener("abort", cancel);
      const cancellations = await Promise.allSettled(signalCancellations);
      flights.delete(accepted.operation.operationId); complete();
      if (cancellations.some(result => result.status === "rejected")) {throw new Error("ordinary signal cancellation persistence unresolved");}
    }
  };
  const submit = (input: OrdinaryInput, options?: OrdinarySubmitOptions): Promise<OrdinarySubmitOutcome> => {
    ...
    void pending.then(() => {submissions.delete(pending);}, () => {submissions.delete(pending);});
    return pending;
  };
