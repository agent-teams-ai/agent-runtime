...
export class ContainedTurnPostgresIntentStore {
  ...
  public async admission(client: PoolClient, key: IntentKey): Promise<"clear" | "denied" | "fingerprint_conflict"> {
    ...
    if (guard !== undefined) {
      ...
      // Accepted command replay still observes its original operation. The claim path checks the guard separately.
      return guard.operationId === null ? "denied" : "clear";
    }
  ...
  public async claimAllowed(client: PoolClient, operation: ContainedTurnKernelOperation): Promise<boolean> {
    ...
    const row = await this.readIntent(client, key);
    return row?.operation_id === operation.operationId && await this.readGuard(client, key) === undefined;
  }
  ...
  async #fenceOperation(
    ...
    if (operation !== undefined && operation.terminal.kind !== "final" && operation.cancellation.kind !== "requested") {
      ...
      const fingerprint = containedTurnCancellationFingerprint({ cancellationCommandId: command.preventionCommandId, operationId: operation.operationId, scopeDigest: containedTurnScopeDigest(command.scope) });
      current = mutateContainedTurnOperation(operation, {
        command: { cancellationCommandId: command.preventionCommandId, fingerprint, operationId: operation.operationId, scopeDigest: containedTurnScopeDigest(command.scope) },
    ...
    if (current !== undefined && current.terminal.kind !== "final" && current.dispatch.kind === "claimed") {
      ...
      await this.operations.persist(client, fenced, current);
    }
    return current;
  }
  ...
  public async prevent(input: Parameters<ContainedTurnKernelOperationStore["preventIntent"]>[0]): ReturnType<ContainedTurnKernelOperationStore["preventIntent"]> {
    ...
    assertContainedTurnExactRecord("trusted prevention scope", input.scope, ["projectId", "tenantId"]);
    ...
    try {
      return await this.transactions.write(async client => {
        if (!await this.lock(client, scope)) {return { kind: "denied" as const };}
        const replay = await client.query<ContainedTurnGuardRow>(
          ...
          [scope.tenantId, scope.projectId, command.preventionCommandId],
        );
        ...
        const key = { commandId: command.commandId, commandFingerprint: command.commandFingerprint, scope };
        const row = await this.readIntent(client, key);
        ...
        const operation = row?.operation_id === undefined || row.operation_id === null
          ? undefined : await this.operations.load(client, row.operation_id, true, scope);
        ...
        const current = await this.#fenceOperation(client, operation, command);
        if (row === undefined) {
          await client.query(
            `INSERT INTO agent_execution.contained_turn_intent_v1(tenant_id,project_id,command_id,command_fingerprint,authority_digest,operation_id)
             VALUES ($1,$2,$3,$4,$5,NULL)`,
            [scope.tenantId, scope.projectId, command.commandId, command.commandFingerprint, this.#authorityDigest],
          );
        }
        const receipt = this.#receipt(command, current);
        ...
        await client.query(
          ...
          [scope.tenantId, scope.projectId, command.commandId, command.preventionCommandId, encoded.codecVersion, encoded.json, encoded.digest],
        );
