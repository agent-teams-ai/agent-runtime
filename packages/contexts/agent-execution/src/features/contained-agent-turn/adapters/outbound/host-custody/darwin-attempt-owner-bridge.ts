import {DarwinAttemptOwnerEvents} from "./darwin-attempt-owner-events.js";
export {DarwinAttemptOwnerEvents} from "./darwin-attempt-owner-events.js";
...
function nativeRelease(failure: () => Error | undefined, closed: () => boolean, lose: (error: Error) => void) {
...
export function bindDarwinAttemptOwnerBridge(endpoint: Duplex) {
  ...
  void ready.then(() => {clearTimeout(helloTimer); return;}, () => {clearTimeout(helloTimer); return;});
  ...
  const release = nativeRelease(() => failed, () => events.retainedClosed() !== undefined, lose);
  ...
  return Object.freeze({
    ...
    bindRetainedOwners: retained.bind.bind(retained),
