...
int ae_tree_entry(ae_tree_transaction *t,uint32_t ordinal,uint32_t parent,
  ...
  int p=parent==AE_TREE_ROOT ? t->root : t->nodes[parent].fd;
  if (!same_node(t->root,t->dev,t->ino,1)) return fail(t);
  if (directory && mkdirat(p,name,0700)!=0) return fail(t);
...
int ae_tree_finish(ae_tree_transaction *t) {
  ...
  for (uint32_t i=t->count;i>0;i--) {
    ...
    int p=n->parent==AE_TREE_ROOT ? t->root : t->nodes[n->parent].fd;
    struct stat st;
    if (fstatat(p,n->name,&st,AT_SYMLINK_NOFOLLOW)!=0 || st.st_dev!=n->dev || st.st_ino!=n->ino) return fail(t);
    if (n->directory && (!same_node(n->fd,n->dev,n->ino,1) ||
        fchmod(n->fd,(mode_t)n->mode)!=0 || !tree_sync(n->fd))) return fail(t);
  }
