...
type ArtifactCustody = Awaited<ReturnType<typeof bindArtifactCustody>>;
...
interface NodeContainedTurnArtifactRuntime {
  ...
  readonly limits: ContainedTurnWorkspaceTreeLimits;
  readonly options: NodeContainedTurnArtifactOptions;
  readonly rehydrationRoots: ArtifactCustody["rehydrationRoots"];
  readonly resultPublications: ArtifactCustody["resultPublications"];
  readonly verifyArtifact: ArtifactStore["verifyArtifact"];
  readonly workspaceRoots: ArtifactCustody["workspaceRoots"];
  readonly writeContentAddressed: ArtifactStore["writeContentAddressed"];
}

const verifyLookup = (
  runtime: NodeContainedTurnArtifactRuntime,
  ...
  workspaceSeals: runtime.workspaceRoots.seals,
});
...
const resolveKernelWorkspace = async (
  ...
  await revalidateBoundRoots(runtime.custodyRoots);
  const name = kernelWorkspaceName(input.workspaceId);
  const [creations] = await openBoundDirectories([runtime.workspaceRoots.creations]);
  try {
    ...
    return Object.freeze({
      ...
      workspaceRef: join(runtime.workspaceRoots.active.canonicalPath, name),
    });
  } finally {await closeContainedTurnArtifactHandles([creations]);}
};

const replayRehydration = (
  runtime: NodeContainedTurnArtifactRuntime,
  lookup: NodeContainedTurnArtifactLookup,
): Promise<string> => guardContainedTurnFilesystemOperation(
  "artifact_rehydrate",
  async () => rehydrateContainedTurnArtifact(lookup.resultRef, {
    ...
    faults: runtime.options.testFaults,
    limits: runtime.limits,
    roots: runtime.rehydrationRoots,
    verifyArtifact: async () => verifyLookup(runtime, lookup),
  }),
  runtime.options.testFaults !== undefined,
);
...
const sealDurably = async (
  ...
  const existing = runtime.inflightSeals.get(input.workspaceRef);
  ...
  const promise = guardContainedTurnFilesystemOperation(
    ...
    () => sealContainedTurnArtifact(input, {
      ...
      writeContentAddressed: runtime.writeContentAddressed,
    }),
    runtime.options.testFaults !== undefined,
  );
...
const readKernelBinding = async (
  ...
  const [results] = await openBoundDirectories([runtime.resultPublications]);
  try {
    let bytes: Buffer;
    try {
...
const queryKernelSeal = async (
  ...
  const resolved = await resolveKernelWorkspace(runtime, input);
  const record = await readKernelBinding(runtime, input);
  if (record === undefined) {
    return Object.freeze({
...
const ensureKernelSealed = async (
  ...
  const observed = await queryKernelSeal(runtime, input);
  if (observed.kind !== "indeterminate") {return observed;}
  const resolved = await resolveKernelWorkspace(runtime, input);
  const sealed = await sealDurably(runtime, {
    operationId: resolved.operationId,
    output: input.output,
    scope: resolved.scope,
  ...
  const [results, staging] = await openBoundDirectories([
    runtime.resultPublications, runtime.workspaceRoots.staging,
  ]);
  try {
  ...
  return queryKernelSeal(runtime, input);
};

const rehydrateLookup = async (
  runtime: NodeContainedTurnArtifactRuntime,
  lookup: NodeContainedTurnArtifactLookup,
): Promise<string> => {
  const digest = parseContainedTurnResultUrnDigest(lookup.resultRef);
  const key = `${runtime.rehydrationRoots.results.canonicalPath}\0${digest}`;
  ...
  if (existing !== undefined) {
    ...
    return replayRehydration(runtime, lookup);
  }
  const promise = replayRehydration(runtime, lookup);
...
const createArtifactApi = (
  runtime: NodeContainedTurnArtifactRuntime,
): NodeContainedTurnArtifacts => {
  ...
  function seal(
    ...
    if ("scope" in input) {return sealDurably(runtime, input);}
  ...
  return Object.freeze<NodeContainedTurnArtifacts>({
    ensureSealed: input => guardContainedTurnFilesystemOperation(
      "artifact_ensure_sealed",
      () => ensureKernelSealed(runtime, input),
      runtime.options.testFaults !== undefined,
    ),
    querySeal: input => guardContainedTurnFilesystemOperation(
      "artifact_query_seal",
      () => queryKernelSeal(runtime, input),
      runtime.options.testFaults !== undefined,
    ),
    rehydrate: lookup => rehydrateLookup(runtime, lookup),
    seal,
    verify: lookup => guardContainedTurnFilesystemOperation("artifact_verify", async () => {
      await revalidateBoundRoots(runtime.custodyRoots);
      return (await verifyLookup(runtime, lookup)).manifest;
    }, runtime.options.testFaults !== undefined),
  });
};

const initializeNodeContainedTurnArtifacts = async (
  options: NodeContainedTurnArtifactOptions,
): Promise<NodeContainedTurnArtifacts> => {
  ...
  const store = createContainedTurnArtifactStore({
    ...
    limits,
    roots: custody.artifactRoots,
    testDigest: options.testDigest,
  });
  return createArtifactApi({
    contentDigest: store.contentDigest,
    ...
    verifyArtifact: store.verifyArtifact,
    workspaceRoots: custody.workspaceRoots,
    writeContentAddressed: store.writeContentAddressed,
  });
};
...
export const createNodeContainedTurnArtifacts = async (
  ...
  "artifact_initialize",
  () => initializeNodeContainedTurnArtifacts(options),
