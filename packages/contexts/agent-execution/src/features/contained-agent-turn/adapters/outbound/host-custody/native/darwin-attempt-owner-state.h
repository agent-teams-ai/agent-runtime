...
typedef enum { AE_EMPTY=AE_PHASE_EMPTY, AE_RESERVED=AE_PHASE_RESERVED, AE_STAGED=AE_PHASE_STAGED,
  AE_START_CONSUMED=AE_PHASE_START_CONSUMED, AE_CHILD_OWNED=AE_PHASE_CHILD_OWNED,
  AE_EXIT_PROVED=AE_PHASE_EXIT_PROVED, AE_NO_START=AE_PHASE_NO_START, AE_DISPOSED=AE_PHASE_DISPOSED,
  AE_RELEASED=AE_PHASE_RELEASED, AE_QUARANTINED=AE_PHASE_QUARANTINED } ae_phase;
typedef enum { AE_ACTIVE=AE_WORKSPACE_ACTIVE, AE_FROZEN=AE_WORKSPACE_FROZEN,
  AE_CLEANUP=AE_WORKSPACE_CLEANING, AE_CLOSED=AE_WORKSPACE_CLOSED } ae_workspace;
typedef struct {
  ae_phase phase;
...
typedef enum { AE_REFUSED=AE_RESULT_REFUSED, AE_ACCEPTED=AE_RESULT_ACCEPTED,
  AE_EFFECT_REQUIRED=AE_RESULT_EFFECT_REQUIRED, AE_UNKNOWN=AE_RESULT_UNKNOWN } ae_result;
...
/* Native-only effect completions. These validate event ordering; only the
 * direct native owner supplies observations, never Host command booleans. */
ae_result ae_birth_observed(ae_state *, ae_persist, void *);
ae_result ae_preexec_observed(ae_state *, ae_persist, void *);
ae_result ae_wait_observed(ae_state *, int, int, ae_persist, void *);
ae_result ae_streams_observed(ae_state *, ae_persist, void *);
