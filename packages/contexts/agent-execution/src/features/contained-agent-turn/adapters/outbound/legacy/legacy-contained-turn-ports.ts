...
import type { ContainedTurnProvider, ContainedTurnScope } from "../../../domain/contained-turn-authority.js";
...
export interface ContainedTurnOperationStore {
  ...
  claimDispatch(input: { readonly cutoffReceiptRef: string; readonly expectedRevision: number; readonly operationId: string }): Promise<ClaimContainedTurnDispatchOutcome>;
  compareAndSet(input: { readonly expectedRevision: number; readonly mutation: ContainedTurnMutation; readonly operationId: string }): Promise<CompareAndSetContainedTurnOutcome>;
  preventDispatch(input: { readonly expectedRevision: number; readonly operationId: string; readonly proofRef: string }): Promise<CompareAndSetContainedTurnOutcome>;
  read(operationId: string): Promise<ContainedTurnOperation | undefined>;
  requestCancellation(input: { readonly expectedRevision: number; readonly operationId: string }): Promise<CompareAndSetContainedTurnOutcome>;
  terminalize(input: { readonly expectedRevision: number; readonly operationId: string }): Promise<CompareAndSetContainedTurnOutcome>;
}

export interface ContainedTurnSecurityPort {
  authorize(input: { readonly intent: ContainedTurnOperation["intent"]; readonly provider: ContainedTurnProvider; readonly scope: ContainedTurnScope }): Promise<
    | { readonly kind: "allowed"; readonly authorityRevision: string; readonly decisionDigest: string }
    | { readonly kind: "denied" }
  >;
  revalidate(input: { readonly authorityRevision: string; readonly decisionDigest: string; readonly operationId: string; readonly scope: ContainedTurnScope }): Promise<
...
export interface ContainedTurnWorkspacePort {
  ...
  create(input: { readonly operationId: string; readonly scope: ContainedTurnScope }): Promise<{ readonly workspaceRef: string }>;
  quarantine(input: { readonly evidenceRef: string; readonly workspaceRef: string }): Promise<void>;
}

export interface ContainedTurnArtifactPort {
  seal(input: { readonly operationId: string; readonly output: readonly { readonly cursor: number; readonly kind: ContainedTurnOutputKind; readonly text: string }[]; readonly workspaceRef: string }): Promise<{
...
export interface ContainedTurnCustodyHandle { readonly custodyRef: string }

export interface ProviderProcessCustodyPort {
  open(input: { readonly attemptId: string; readonly operationId: string; readonly providerBinding: ContainedTurnProviderBinding; readonly workspaceRef: string }): Promise<ContainedTurnCustodyHandle>;
  requestContainment(input: { readonly attemptId: string; readonly custodyRef?: string; readonly operationId: string }): Promise<
